...
func (s *mockListener) EnterUpdate_statement(ctx *Update_statementContext) {
	...
	for i, colSet := range allColSets {
		...
		case emptyClobStr:
			...
			if _, ok := treeNode.(*ExpressionContext); ok {
...
func (s *mockListener) EnterInsert_statement(ctx *Insert_statementContext) {
	...
	for i, col := range colNames {
		...
		case emptyClobStr:
			...
			if _, ok := treeNode.(*ExpressionsContext); ok {
				s.kv[colName] = &emptyClob{}
			}
		case nullStr:
			s.kv[colName] = nil
	...
	datadriven.Walk(t, testdataPath, func(t *testing.T, path string) {
		datadriven.RunTest(t, path, func(t *testing.T, td *datadriven.TestData) string {
			...
			case "parse":
				...
				lexer := NewPlSqlLexer(antlr.NewInputStream(td.Input))
