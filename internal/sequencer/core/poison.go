...
func (p *poisonSet) IsPoisoned(batch *types.MultiBatch) bool {
	// Read-locked check.
	toContaminate, isPoisoned := p.isPoisoned(batch)
	if !isPoisoned {
		return false
	}
...
// isPoisoned returns a set of keys to contaminate.
func (p *poisonSet) isPoisoned(batch *types.MultiBatch) (map[poisonedKey]struct{}, bool) {
	p.mu.RLock()
	defer p.mu.RUnlock()
	...
	// Accumulate keys which may be poisoned.
	var isPoisoned bool
	...
	return toContaminate, isPoisoned
}
