...
func (s *BestEffort) sweepTable(
	...
	ctx.Go(func() error {
		_, err := stopvar.DoWhenChanged(ctx, nil, partialProgress, func(ctx *stopper.Context, _, partial *Stat) error {
			// The timestamp is fake, so we don't want to report
			// anything.
			if partial.Synthetic {
				return nil
			}
			...
			_, _, err := stats.Update(func(old sequencer.Stat) (sequencer.Stat, error) {
				...
				// We want to export counters for testing.
				nextImpl.Attempted += partial.Attempted
				nextImpl.Applied += partial.Applied
				nextImpl.Synthetic = partial.Synthetic
				nextImpl.Progress().Put(table, partial.LastTime)
				return nextImpl, nil
			})
			return err
		})
		...
		_, err := stopvar.DoWhenChangedOrInterval(ctx, hlc.Range{}, bounds, s.cfg.QuiescentPeriod,
			func(ctx *stopper.Context, _, bound hlc.Range) error {
				...
				if bound.Empty() {
					...
					partial.Synthetic = true
					if bound.Empty() {
						return nil
					}
				}
				partialProgress.Set(partial)
				if err := s.sweepOnce(ctx, table, bound, acceptor, partialProgress); err != nil {
...
type Stat struct {
	...
	Synthetic bool     // True if we've picked an arbitrary time to make progress with.
}
...
func (s *BestEffort) sweepOnce(
	...
	for hasMore := true; hasMore && !ctx.IsStopping(); {
		...
		// This accumulates the timestamps of any failed mutations.
		...
		if earliestFailed.Nanos() != math.MaxInt64 {
			...
			stat.LastTime = earliestFailed
		} else if hasMore {
			// We did not see any failed mutations within this
			// iteration, but there may be additional mutations to
			// unstage later. We'll report the timestamp to which the
			// table has advanced.
			stat.LastTime = q.TableOffsets.GetZero(tbl).Time
		...
		//		stat.LastTime = hlc.New(stat.LastTime.Nanos()-1, 0)
