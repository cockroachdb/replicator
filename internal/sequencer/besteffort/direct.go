...
// target table or will stage them.
...
func (a *directAcceptor) AcceptTableBatch(
	...
	outcome := a.scheduler.TableBatch(batch, func() error {
		...
		// All mutations were already blocked by another mutation. We
		// need to commit the staging transaction here, since the
		// StageIfExists call may have written data.
		if len(attempt) == 0 {
			return errors.WithStack(stageTx.Commit(ctx))
		}

		// Make a copy of the batch so we have the original to pass into
		// the fallback.
