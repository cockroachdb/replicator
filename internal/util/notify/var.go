...
// Get returns the value last passed to Set or the zero value for T and
// a channel that will be closed the next time that Set is called. This
// API does not guarantee that a loop as shown below will see every
// update made to the Var.  Rather, it allows a consumer to sample
// the most current value available.
//
