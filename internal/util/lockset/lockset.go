		...
		defer func() {
			...
			case *retryAtHead:
				...
				if s.mu.head == w {
					...
					// waiter was already at the head of the global
					// queue, since there's nothing else to pump the
					// event loop.
					w.result.Set(&Status{err: t.Unwrap()})
				} else {
					// Otherwise, re-enable the waiter. The status will
					// be set to retryRequested for later re-dispatching
					// by the dispose method.
					w.fn = fn
