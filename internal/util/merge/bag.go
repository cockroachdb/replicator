...
type Bag struct {
	...
	Mapped   *ident.Map[*Entry]      // Properties present in Columns.
...
func NewBag(colData []types.ColData, rename *ident.Map[ident.Ident]) *Bag {
	ret := &Bag{
		Columns:  colData,
	...
	// Seed the mapped columns with empty entries.
...
func NewBagFrom(b *Bag) *Bag {
	return NewBag(b.Columns, b.Rename)
}
...
// Range implements cmap.Map. It will emit the values of all mapped
// properties, even if they are nil.
