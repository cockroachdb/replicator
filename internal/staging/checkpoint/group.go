...
//   - p_max_times: Determines the latest available checkpoint for each
//     partition within the group.
//   - visible_data: Restricts available_data by the minimum-maximum
//     value from p_max_times.
//   - p_max_unapplied: Finds the last checkpoint time that hasn't bee
//     processed.
...
SELECT *
  FROM available_data
 WHERE source_hlc <= (SELECT min(hlc) FROM p_max_times)
),
p_max_unapplied AS (
SELECT partition, max(source_hlc) AS hlc
  FROM visible_data
...
stop AS (
SELECT min(COALESCE(p_max_unapplied.hlc, (SELECT hlc FROM last_applied))) AS hlc
  FROM (SELECT DISTINCT partition FROM visible_data)
  LEFT JOIN p_max_unapplied USING (partition)
),
look_back AS (
SELECT max(source_hlc) AS hlc
  FROM visible_data
