...
type mergeBuffer struct {
	buffer    []*types.TemporalBatch // Data from the upstream channels
	bufferIdx int                    // Index into buffered TemporalBatches
	fragment  bool                   // True if the last batch is fragmented.
	jump      bool                   // Indicates that the source was discontiguous.
	progress  hlc.Range              // A zero-based range showing processed timestamps.
...
func (s *mergeBuffer) Empty() bool {
	return s.bufferIdx >= len(s.buffer)
}
...
func (s *mergeBuffer) Progress() hlc.Range {
	...
	return hlc.RangeExcluding(hlc.Zero(), s.buffer[s.bufferIdx].Time)
}

// Refresh the buffer from the underlying channel. If the context is
// null, the refresh will be non-blocking.
...
func (s *mergeBuffer) ingest(cursor *tableCursor) error {
	...
	s.buffer = cursor.Batch.Data
	s.bufferIdx = 0
	s.fragment = cursor.Fragment
	s.jump = cursor.Jump
	s.progress = cursor.Progress
	return nil
}
...
func (m *tableMerger) run(ctx *stopper.Context) {
	...
	for {
		...
		cursor, refill, blocking := m.nextStep(ctx)
...
// to send.
func (m *tableMerger) nextStep(
	ctx *stopper.Context,
) (cursor *types.StagingCursor, refill []*mergeBuffer, block bool) {
