...
func (t *Config) Equal(o *Config) bool {
	if !reflect.DeepEqual(t.CASColumns, o.CASColumns) {
		return false
	}
	if !t.Deadlines.Equal(o.Deadlines, cmap.Comparator[time.Duration]()) {
		return false
	}
	if !t.Exprs.Equal(o.Exprs, cmap.Comparator[string]()) {
		return false
	}
	if !ident.Equal(t.Extras, o.Extras) {
		return false
	}
	if !t.Ignore.Equal(o.Ignore, cmap.Comparator[bool]()) {
		return false
	}
	if !t.SourceNames.Equal(o.SourceNames, ident.Comparator[ident.Ident]()) {
		return false
	}
	return true
}
