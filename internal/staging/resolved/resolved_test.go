...
func TestResolved(t *testing.T) {
	...
	g1, err := fixture.Resolved.Start(ctx,
	...
	g2, err := fixture.Resolved.Start(ctx,
		&types.TableGroup{Name: ident.New("fake")},
		bounds2,
	)
	r.NoError(err)
	...
	t.Run("mark-and-refresh", func(t *testing.T) {
		...
		for i := minNanos; i <= maxNanos; i++ {
			r.NoError(g1.Mark(ctx, hlc.New(i, 0)))
		}
	...
	t.Run("record-and-refresh", func(t *testing.T) {
		r := require.New(t)
		// Setting logical to 1 since end is exclusive; makes the tests below less awkward.
		r.NoError(g2.Record(ctx, hlc.Range{hlc.New(minNanos, 0), hlc.New(maxNanos, 1)}))
	...
	t.Run("no-going-back", func(t *testing.T) {
		r := require.New(t)
		r.ErrorContains(g1.Mark(ctx, hlc.New(1, 1)), "is going backwards")
	})
}
