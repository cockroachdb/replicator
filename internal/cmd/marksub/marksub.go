...
//
// This function is implemented as a state machine using the various
// mode types as the states and temporary storage.
//   - modeFindCommand: Write each line of input to the output, while
//     looking for a marksub command comment (via commandPattern). When a
//     command has been found, it switches to modeLookAhead.
//   - modeLookAhead: Continue to copy input to output, but look for the
//     next code-block comment (via blockPattern). When the start of the
//     code block has been found, it will be written and the contents of
//     the commanded file are copied into the output. We then switch to
//     modeDiscard.
//   - modeDiscard: Ignore input lines until the closing code-block
//     marker is found. The closing marker will be written to the output.
//     We then switch to modeFindCommand.
func substitute(basePath string, path string, out io.Writer) error {
	...
	for reading := true; reading; {
		...
		case modeDiscard:
			// Discard input until we see the closing marker.
