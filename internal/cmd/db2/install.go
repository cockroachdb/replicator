...
func Install() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Use:   "db2install",
		// We are hiding it, since it's mainly used for testing.
		Hidden: true,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if os.IsNotExist(err) || os.IsNotExist(includeErr) || os.IsNotExist(libErr) {
				pkgName, err := install(cmd.Context(), dest, downloadURL, dryrun)
				if err != nil {
					return err
				}
				if pkgName != "" {
	...
	cmd.Flags().BoolVar(&dryrun, "dryrun", false,
		"show the instructions to install the package manually")
...
func install(ctx context.Context, target, baseURL string, dryrun bool) (string, error) {
	var pkgName string
	switch runtime.GOOS {
	case "darwin":
