...
func (h *Handler) resolved(ctx context.Context, r resolvedURL) error {
	...
	return retry.Retry(ctx, func(ctx context.Context) error {
		...
		targetSchema := watcher.Snapshot()

		// Prepare to merge data.
		stores := make([]sinktypes.MutationStore, 0, len(targetSchema))
		appliers := make([]sinktypes.Applier, 0, len(targetSchema))
		for table := range targetSchema {
			...
			if r.target != ident.NewSchema(table.Database(), table.Schema()) {
				continue
			}
		...
		prev, err := h.Swapper.Swap(ctx, tx, "_resolved_"+r.target.Raw(), r.timestamp)
		if err != nil {
			return err
		}
