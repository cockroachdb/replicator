...
const (
	// Boundary is the folder delimiter used. While Amazon S3 has a flat
...
func (b *s3Bucket) Walk(
	...
	if dir != "" {
		dir = strings.TrimSuffix(dir, Boundary) + Boundary
	}
	after := strings.TrimPrefix(options.StartAfter, b.bucket+Boundary)
...
func (b *s3Bucket) Open(ctx *stopper.Context, file string) (io.ReadCloser, error) {
	...
	file = strings.TrimPrefix(file, b.bucket+Boundary)
