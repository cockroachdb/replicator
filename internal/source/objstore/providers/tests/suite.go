...
// Package tests defines the tests that the providers must pass.
package tests
...
// The error types that we expect in the tests. Providers will supply
// a map that defines the actual error send by the provider for these types.
const (
...
type Suite struct {
	Errors map[Errors]error // Maps error types we expect to actual errors thrown the provider.
...
func (v *Suite) Open(t *testing.T) {
	...
	tests := []struct {
		...
		{"found", "test.txt", "test", ""},
		{"notfound", "nothere.txt", "", v.Errors[NoSuchKey].Error()},
	}
