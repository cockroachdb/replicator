...
func (s mySQLStamp) String() string {
	if s.gtidset == nil {
		return ""
	}
	return s.gtidset.String()
}
func (s mySQLStamp) Less(other stamp.Stamp) bool {
	if o, ok := other.(mySQLStamp); ok {
		...
		log.Tracef("Less: %s  < %s : %t %t", s.String(), o.String(),
			o.gtidset.Contain(s.gtidset), s.gtidset.Equal(o.gtidset))
...
func NewConn(ctx context.Context, config *Config) (_ *Conn, stopped <-chan struct{}, _ error) {
	...
	ret := &Conn{
		...
		sourceConfig:  cfg,
		memoKey:       config.ConsistentPointKey,
	...
	for {
		...
		if ok {
			...
			case *replication.GTIDEvent:
				...
				log.Tracef("GTIDEvent adding %s to %s ", s, c.lastGTIDEvent.String())
				a, err := mysql.ParseUUIDSet(s)
				if err == nil {
					clone := &mysql.MysqlGTIDSet{
						Sets: make(map[string]*mysql.UUIDSet),
					}
					for sid, uuidSet := range c.lastGTIDEvent.gtidset.Sets {
...
func (c *Conn) ReadInto(ctx context.Context, ch chan<- logical.Message, state logical.State) error {
	...
	gtidset, err := mysql.ParseMysqlGTIDSet(state.GetConsistentPoint().String())
	if err != nil {
...
func (c *Conn) UnmarshalStamp(stamp string) (stamp.Stamp, error) {
	...
	s, err := mysql.ParseMysqlGTIDSet(stamp)
	if err != nil {
		return nil, err
	}
	ret, ok := s.(*mysql.MysqlGTIDSet)
	if !ok {
		return nil, errors.New("cannot unmarshal stamp " + stamp)
	}
