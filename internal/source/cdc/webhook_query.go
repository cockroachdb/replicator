...
func (h *Handler) webhookForQuery(ctx context.Context, req *request) error {
	...
	var message struct {
		...
		// With envelope="raw" (default for queries), there is a  `__crdb__` property.
		Raw []json.RawMessage `json:"__crdb__"`
	...
	// Raw messages are not longer supported.
	if message.Raw != nil {
		return errors.New(rawEnvelopeErrorMsg)
	}
