...
var (
	//go:embed testdata/ddl.txt
	ddl string
	//go:embed testdata/delete.txt
	delete string
	//go:embed testdata/insert.txt
	insert string
	//go:embed testdata/snapshot.txt
	snapshot string
	//go:embed testdata/tombstone.txt
	tombstone string
	//go:embed testdata/update.txt
	update string
	// json "null"
...
func TestParseStream(t *testing.T) {
	for _, name := range []string{"mysql", "sqlserver", "oracle"} {
		t.Run(name, func(t *testing.T) {
			a := assert.New(t)
			r := require.New(t)
			file, err := os.Open(fmt.Sprintf("./testdata/%s.txt", name))
			r.NoError(err)
			defer file.Close()
