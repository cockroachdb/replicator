...
func TestDataTypes(t *testing.T) {
	...
	tcs := []struct {
		...
		// box
		{`bytea`, []interface{}{"", "HELLO WORLD!"}},
		{`character`, []interface{}{"", "HELLO WORLD!"}},
		{`character varying`, []interface{}{"", "HELLO WORLD!"}},
		...
		{`json`, []interface{}{"null", `{"hello":"world"}`}},
		{`jsonb`, []interface{}{"null", `{"hello":"world"}`}},
		// line
		// lseg
	...
	for idx, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !a.Equalf(len(tc.values)+1, count, "mismatch in %s", tgts[idx]) {
				runtime.Breakpoint()
			}
		})
	}
