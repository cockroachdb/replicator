...
func (c *Conn) decodeMutation(tbl ident.Table, data *pglogrepl.TupleData) (types.Mutation, error) {
...
// flush commits all pending mutations to their respective stages. It
// will also zero-out the length of the associated pending slice.
...
func (c *Conn) processMessages(ctx context.Context, ch <-chan pglogrepl.Message) error {
	for msg := range ch {
		...
		case *pglogrepl.CommitMessage:
			...
			// Apply the transaction data, through the usual
			// drain-and-apply mechanism.
		...
		case *pglogrepl.DeleteMessage:
			traceTuple(msg.OldTuple)
			tbl, ok := c.relations[msg.RelationID]
			if !ok {
				return errors.Errorf("unknown relation id %d", msg.RelationID)
			}
			mut, err := c.decodeMutation(tbl, msg.OldTuple)
			if err != nil {
				return err
			}
			// We only care about the key data for deletes.
			mut.Data = json.RawMessage{}
			if err := c.stage(ctx, tbl, mut); err != nil {
				return err
			}

		case *pglogrepl.InsertMessage:
			traceTuple(msg.Tuple)
			tbl, ok := c.relations[msg.RelationID]
			if !ok {
				return errors.Errorf("unknown relation id %d", msg.RelationID)
			}
			mut, err := c.decodeMutation(tbl, msg.Tuple)
			if err != nil {
				return err
			}
			if err := c.stage(ctx, tbl, mut); err != nil {
		...
		case *pglogrepl.UpdateMessage:
			traceTuple(msg.NewTuple)
			tbl, ok := c.relations[msg.RelationID]
			if !ok {
				return errors.Errorf("unknown relation id %d", msg.RelationID)
			}
			mut, err := c.decodeMutation(tbl, msg.NewTuple)
			if err != nil {
				return err
			}
			if err := c.stage(ctx, tbl, mut); err != nil {
		...
		case *pglogrepl.TruncateMessage:
			return errors.Errorf("the TRUNCATE operation cannot be supported on table %d", msg.RelationNum)

		default:
			return errors.Errorf("unimplemented logical replication message %T", msg)
		}
	}
...
// stage will add the mutation to the in-memory slice, possibly flushing
// it when the batch size limit is reached.
func (c *Conn) stage(ctx context.Context, tbl ident.Table, mut types.Mutation) error {
	if ctrl := c.testControls; ctrl != nil {
		if fn := c.testControls.BreakSinkStage; fn != nil {
			if fn() {
				return errors.New("breaking stage for test")
			}
		}
	}
	found := c.pending[tbl]
