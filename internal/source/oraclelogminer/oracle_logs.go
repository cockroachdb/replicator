...
func GetChangeFeedLogs(ctx *stopper.Context, db *DB) ([]RedoLog, error) {
	...
	for rows.Next() {
		if err := rows.Err(); err != nil {
			return nil, errors.Wrapf(err, "failed to get changefeed from logMiner")
		}
	...
	if _, err := db.ExecContext(ctx, startLogMiner, db.scn.Val, endSCNLogMinerExec.String); err != nil {
