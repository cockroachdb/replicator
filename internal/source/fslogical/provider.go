...
func ProvideLoops(
	ctx context.Context, cfg *Config, loops *logical.Factory, fs *firestore.Client, st *Tombstones,
...
func ProvideTombstones(
	ctx context.Context, cfg *Config, fs *firestore.Client, loops *logical.Factory,
) (*Tombstones, error) {
	...
	ret.targets = make(map[string]ident.Table, len(cfg.SourceCollections))
	for idx, source := range cfg.SourceCollections {
		ret.targets[source] = cfg.TargetTables[idx]
	}
