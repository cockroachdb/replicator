    ...
    WITH pk_name AS (
         ...
         pks AS (
              SELECT table_catalog,
                ...
                JOIN pk_name USING (table_catalog, table_schema, table_name, constraint_name)
             ),
         cols AS (
                SELECT table_catalog,
                       table_schema,
	...
	err := retry.Retry(ctx, func(ctx context.Context) error {
		...
		for rows.Next() {
			...
			case types.ProductCockroachDB, types.ProductPostgreSQL:
				...
				if len(parts) != 3 {
					return errors.Errorf("unexpected number of parts when splitting ident: %s", parsed)
				}

				lastPart := parts[2].Raw()
				isArray := lastPart[0] == '_'
				if isArray {
					parts[2] = ident.New(lastPart[1:])
				}
