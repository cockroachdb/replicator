...
func (a *apply) upsertBagsLocked(
	...
	if mode == applyUnconditional || merger == nil {
		...
		// Conflicts could be non-zero if CAS is enabled, but there's no
		// merge function.
	...
	log.WithFields(log.Fields{
		...
		"target":    a.target,
		"upserted":  len(bags),
	}).Debug("conditionally upserted rows")
...
// newBackLocked constructs a new property bag using cached metadata.
func (a *apply) newBagLocked() *merge.Bag {
	return merge.NewBag(a.mu.templates.Columns, a.mu.templates.Renames)
}
